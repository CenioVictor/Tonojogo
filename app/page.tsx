...
import { GameState, Piece, BeatType } from '@/lib/types';
...
export default function Home() {
  ...
  const playableCards = currentPlayer.pieces.filter(
    (piece) => canPlayPiece(piece, gameState.board)
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-600 to-green-800 p-4">
      ...
      <div className="bg-white rounded-lg p-4 mb-6 shadow-lg">
        ...
        <p className="text-sm text-gray-600">
          Peças no Monte: {gameState.pile.length}
        </p>
        {gameState.closedGame && (
          <p className="text-red-600 font-bold">🔒 JOGO FECHADO!</p>
        )}
      </div>
        ...
        <div className="flex gap-2 flex-wrap justify-center">
          {gameState.board.length === 0 ? (
            ...
            gameState.board.map((piece, idx) => (
              <div
                key={idx}
                className="bg-yellow-600 text-white rounded-lg p-3 shadow-lg font-bold text-center"
              >
          ...
          .map((player) => (
            <div
              ...
              className={`rounded-lg p-4 ${
                player.id === currentPlayer.id ? 'bg-green-500' : 'bg-gray-600'
              } text-white`}
            >
